...
class system:
    ...
    def phi(self, current_time):
        #original phi, a little weird I think
        phi = self.a * self.F0 / (2. * np.pi * self.field) * (
                    np.sin(np.pi * current_time / self.cycles) ** 2.) * np.sin(2. * np.pi*current_time)

        # # #Trying one that seems to be consistent with paper.
        # phi = self.a * self.F0 / (self.field) * (
        #             np.sin(np.pi * current_time / self.cycles) ** 2.) * np.sin(self.field*current_time)
        return phi
